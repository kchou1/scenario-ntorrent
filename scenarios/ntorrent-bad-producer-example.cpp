...
main(int argc, char *argv[])
{
  ...
  AnimationInterface::SetConstantPosition (nodes.Get(2), 20, 60);
  ...
  // Bad producer (satisfies only 10% of all interests)
  ndn::AppHelper badProducerHelper("NTorrentProducerApp");
  badProducerHelper.SetAttribute("Prefix", StringValue("/"));
  badProducerHelper.SetAttribute("namesPerSegment", IntegerValue(namesPerSegment));
  badProducerHelper.SetAttribute("namesPerManifest", IntegerValue(namesPerManifest));
  badProducerHelper.SetAttribute("dataPacketSize", IntegerValue(dataPacketSize));
  badProducerHelper.SetAttribute("probability", IntegerValue(10));
  badProducerHelper.Install(nodes.Get(2)).Start(Seconds(2.0));
