...
main(int argc, char *argv[])
{
  ...
  StrategyChoiceHelper::Install(nodes.Get(3), "/beacon", "/localhost/nfd/strategy/broadcast/%FD%01");
  StrategyChoiceHelper::Install(nodes.Get(3), "/bitmap", "/localhost/nfd/strategy/broadcast/%FD%01");
  StrategyChoiceHelper::Install(nodes.Get(3), "/movie1", "/localhost/nfd/strategy/broadcast/%FD%01");
  StrategyChoiceHelper::Install(nodes.Get(3), "/movie2", "/localhost/nfd/strategy/broadcast/%FD%01");
  ndn::AppHelper p4("NTorrentAdHocForwarder");
  ...
  FibHelper::AddRoute(nodes.Get(3), "/movie1", std::numeric_limits<int32_t>::max());
  FibHelper::AddRoute(nodes.Get(3), "/movie2", std::numeric_limits<int32_t>::max());

  // right most
  ndn::AppHelper p5("NTorrentAdHocAppNaive");
  p5.SetAttribute("TorrentPrefix", StringValue("movie2"));
  p5.SetAttribute("NumberOfPackets", StringValue("10"));
  p5.SetAttribute("NodeId", StringValue("1"));
  p5.SetAttribute("BeaconTimer", StringValue("1s"));
  p5.SetAttribute("RandomTimerRange", StringValue("20ms"));
  p5.SetAttribute("TorrentProducer", BooleanValue(false));
  ApplicationContainer peer5 = p5.Install(nodes.Get(4));
  peer5.Start(Seconds(0));
  FibHelper::AddRoute(nodes.Get(4), "/beacon", std::numeric_limits<int32_t>::max());
  FibHelper::AddRoute(nodes.Get(4), "/bitmap", std::numeric_limits<int32_t>::max());
  FibHelper::AddRoute(nodes.Get(4), "/movie1", std::numeric_limits<int32_t>::max());
  FibHelper::AddRoute(nodes.Get(4), "/movie2", std::numeric_limits<int32_t>::max());
